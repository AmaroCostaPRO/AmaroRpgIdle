...
*   **🐛 Correções pós-lançamento**: (1) Coral zerado a cada 60s quando havia Expedições da Cidadela alocadas — `tickCitadelProduction` reconstruía `materials` sem spread (`useGameStore.ts`). (2) Bestiário só exibia 2 dos 8 grupos novos (Litoral + Zona 1) — a UI (`GameUI.tsx`) e o cálculo de bônus (`StatEngine.calculateBestiaryDamageMultiplier`) usavam listas hardcoded independentes e desatualizadas; unificadas em `bestiaryFormulas.ts` (`BESTIARY_PHASE_GROUPS`, agrupamento por ID em vez de recorte posicional do array `ENEMY_TYPES`). (3) Inimigos exclusivos do Abismo vazando para os sorteios aleatórios da Torre Infinita e do Pandemônio — `ABYSS_ENEMY_IDS` (`CombatFSM.ts`) agora filtra os 4 pontos de sorteio (comum/chefe × Torre/Pandemônio). (4) Layout dos cartões de isca do Litoral desalinhado no mobile — `CoastalPanel.tsx` trocado de `flex-wrap` para `grid` com colunas uniformes. (5) Coral também reduzido a 2% na Ascensão junto com os demais materiais farmados da run (`performPrestige`, `useGameStore.ts`) — por ser um recurso raro de se farmar nas Profundezas/Litoral, passou a ser a única exceção do bloco e agora sobrevive 100% intacto (Transcendência continua zerando Coral normalmente, como "poder do ciclo"). (6) Sair voluntariamente de um Mergulho Raso deixava a flag `isAirPocketEncounter` (`CombatFSM.ts`) presa em `true` — o branch de campanha de `setupEnemyForLevel` resetava as demais flags de contexto de encontro (`isElite`, `isMerchantEncounter`, `isConvergenceEncounter`, `isCoastalEncounter`) mas esquecia essa, travando o scroll lateral (o inimigo nascia já em alcance de combate) e exibindo o nome/cor do "Bolsão de Ar" sobre o inimigo real até um recarregamento completo da página.
