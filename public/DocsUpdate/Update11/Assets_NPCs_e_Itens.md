...
| `npc_forge_master_vulkan.png` | Vulkan, o Mestre da Forja | Cidadela Astral | **Prompt**: High density 512-bit pixel art bust portrait (chest-up, 3/4 angle) of Vulkan, the Master Forge Dwarf NPC. A muscular dwarven smith with a glowing fiery ember beard, wearing a heavy leather apron with glowing ember runes, warhammer resting on one shoulder (head of hammer visible near frame edge). Crisp black outlines, micro-texture dithering. Flat solid pure red background `#FE0201` with NO glow outside and no blur at the edges. No drop shadow. |
