...
### C. Estrutura de Navegação: Categorias + Sub-abas (reformulado na v11.4.0)
O jogo cresceu de um punhado de abas para 17 abas de topo (mais as sub-abas internas da Cidadela/Cidadela Submersa), tornando a fileira única de abas cansativa de navegar, especialmente durante o combate no mobile. A v11.4.0 reformulou a navegação em dois níveis, sem alterar o conteúdo de nenhum painel:

*   **Combate** continua sendo um atalho isolado, sempre acessível em 1 clique/toque, fora do sistema de categorias — é o único item sem sub-abas.
*   As demais 16 abas foram agrupadas em **5 categorias** definidas em `src/components/nav/navConfig.ts` (`NAV_CATEGORIES`): **Personagem** (Equipamento, Atributos, Habilidades, Jornada), **Atividades** (Torre, Abismo, Cidadela), **Evolução** (Forja, Ascensão, Transcendência, Loja), **Codex** (Guia, Bestiário, Codex) e **Opções** (Saves, Configurações — nomes trocados na v11.4.0: a categoria que reúne os dois é "Opções", e a sub-aba de preferências do jogador em si passou a se chamar "Configurações", ver Seção 3.E).
*   **Estado de navegação** (`GameUI.tsx`): o antigo `activeTab` (união de 17 valores) virou um par `activeCategory` (categoria aberta, ou `null` para Combate) + `activeSubTab` (um `Record` guardando, por categoria, qual sub-aba foi vista por último). `activeTab` continua existindo como valor **derivado** dos dois, então todo o corpo de renderização de cada painel (que compara `activeTab === 'x'`) não precisou mudar. A troca de aba passou a ser feita por um helper único, `navigateTo(tabId)`, que resolve a categoria dona da aba via `findCategoryForTab()` e atualiza os dois estados juntos.
*   **`CategorySidebar.tsx` (desktop, `>840px`)**: rail lateral com estilo de painel (`.panel`, cantos arredondados, mesmo espaçamento dos demais cards), posicionado à **direita** da área de conteúdo das abas (não entre ela e a tela de combate). Largura fixa (`~6.4rem`); cada categoria mostra só o ícone centralizado, exceto a categoria aberta, que mostra ícone em cima e o nome embaixo.
*   **`MobileNavSheet.tsx` (mobile, `≤840px`)**: substituiu uma barra inferior fixa (que chegou a existir e foi removida por ocupar espaço duplicado com o rodapé "Sair"/"Voltar") por uma folha modal que sobe de baixo, aberta por um botão ☰ inserido no canto esquerdo do próprio rodapé (`header-panel`). Renderizada via `createPortal` com scrim de fundo; fecha tocando fora, tocando um item, ou automaticamente ao trocar de categoria, com animação de abertura (`nav-sheet-slide-up`/`nav-sheet-fade-in`) e uma animação de fechamento simétrica e invertida (`nav-sheet-slide-down`/`nav-sheet-fade-out`, 200ms) — o componente só desmonta depois que a transição termina, evitando um "sumiço" abrupto. Não tem título nem botão de fechar dedicado (intencionalmente enxuto: tocar fora já fecha).
*   **`SubTabBar.tsx`**: componente genérico (setas + janela deslizante no desktop, carrossel giratório no mobile) extraído do padrão que já existia em `CitadelTabsBar.tsx`/`SunkenCitadelTabsBar.tsx` (que viraram wrappers finos dele). Usado tanto para navegar entre as sub-abas de uma categoria aberta quanto — sem nenhuma mudança de comportamento — para as sub-abas internas da Cidadela e da Cidadela Submersa, que continuam substituindo inteiramente a barra por cima enquanto o jogador está "dentro" delas. Sub-abas sempre mostram o nome (diferente do sidebar de categorias, que só mostra nome na ativa); a janela do desktop na Cidadela foi reduzida de 5 para 4 abas visíveis por vez (`CitadelTabsBar.tsx`), já que a coluna de conteúdo ficou mais estreita com a chegada do sidebar.
*   **Badges de notificação por categoria**: `useTabNotifications` (Seção 3.J) já calculava a bolinha por aba; uma nova função em `GameUI.tsx` (`categoryHasNotification`) agrega essas mesmas condições — uma categoria acende a bolinha se **qualquer** aba interna tiver algo pendente. O botão ☰ do mobile também acende se qualquer uma das 5 categorias tiver pendência.
*   **Conteúdo de cada painel** (sem mudança de comportamento, só de localização na árvore de navegação):
    ...
    *   **Configurações** (sub-aba; antes chamada "Opções" — nome trocado com a categoria na v11.4.0): Centraliza configurações do jogo e preferências de Qualidade de Vida (QoL) do jogador, incluindo áudio, console de combate, formatação de números, auto-venda de equipamentos dropados e controle do robô assistente. Desde a v6.1.0, é dividida em duas sub-abas internas (`⚙️ Configurações` / `📊 Estatísticas`, estado `statsSubTab` em `OptionsPanel`) — a primeira preserva 100% do conteúdo pré-existente, a segunda expõe o painel de Estatísticas Completas (ver Seção 3.E.4).
...
### E. Configurações do Jogo e Qualidade de Vida (QoL)
A sub-aba **Configurações** (dentro da categoria "Opções" — nomes trocados na v11.4.0, ver Seção 3.C) centraliza recursos voltados a personalizar a experiência de jogo e automatizar tarefas repetitivas, salvando as preferências do usuário localmente em `localStorage`.
...
### F. Navegação por Gestos (Mobile Swipe) — escopo ajustado na v11.4.0
Para melhorar a experiência de usabilidade em dispositivos móveis, foi adicionado suporte a gestos de arrastar (*swipe*) horizontal no painel de interface principal (`game-ui-root`). Com a reformulação de navegação (Seção 3.C), o alvo do swipe deixou de ser a lista completa de 17 abas e passou a ser as **sub-abas da categoria aberta no momento**:
*   **Sem categoria aberta (Combate)**: swipe não faz nada — trocar de categoria é só pelo sidebar (desktop) ou pela folha de navegação (mobile), nunca por swipe.
*   **Categoria aberta (ex.: Personagem)**: swipe para a esquerda avança para a próxima sub-aba à direita (ex.: de *Equipamento* para *Atributos*); swipe para a direita volta para a sub-aba anterior. Nas pontas da lista, o swipe é **travado** (não dá a volta para a última/primeira sub-aba de outra categoria) — mudar de categoria continua sendo só pelo sidebar/folha.
*   **Dentro da Cidadela ou Cidadela Submersa** (`citadelEntered`/`sunkenEntered`): comportamento inalterado — o swipe continua ciclando as sub-abas internas delas (com voltas nas pontas), já que são um overlay de imersão à parte da navegação por categorias.
...
*   **Celestial (`celestial`)**: Equipamento de tier supremo, com dois requisitos de desbloqueio simultâneos (v-next): derrotar o chefe do Purgatório (`boss_crystal_guardian`) pela segunda vez em diante **e** estar na Fase 31+ (Pandemônio). Antes dependia só do abate do chefe, permitindo obter o bônus de 5 peças mais forte do jogo já nas fases iniciais e ofuscando os demais sets — agora só passa a dropar de fato no endgame. Possui 10% de chance de substituir os drops normais nessa faixa de fase. Concede bônus em **3 atributos** de escala divina com multiplicador de atributos de **$6.0\times$**. Os itens deste set recebem um bônus especial de **$2.0\times$** em seu valor de venda por ouro.
...
*   **Fase 31+ (Pandemônio) — Apenas Itens Lendários (ou superiores)**: A partir da Fase 31, itens de raridade Comum e Raro deixam de dropar por completo — todo drop de equipamento é obrigatoriamente Lendário (ou uma de suas variantes de conjunto: Ancestral, Pandemoníaco, Lua de Sangue, Celestial). Reflete o fato de que peças fracas deixam de ser relevantes no Pandemônio. **(v-next)** A raridade/set dessa faixa de fase é resolvida filtrando as opções elegíveis **antes** de rolar — Pandemoníaco está sempre elegível a partir daqui, servindo de resultado garantido — em vez de sortear livremente e descartar o drop já concedido quando o resultado não servisse para a fase (bug corrigido: um slot que passasse na rolagem de `dropChance` podia ainda assim não gerar item nenhum).
...
*   **Sets Celestiais (Tier Supremo - Pós-Purgatório Fase 30, Fase 31+)**:
    Estes conjuntos representam a progressão máxima de endgame e são liberados após vencer o Guardião dos Cacos duas ou mais vezes no Purgatório **e** alcançar a Fase 31+ (v-next — antes só o abate do chefe já liberava o drop, mesmo em fases iniciais). Possuem atributos celestiais e impulsionam ao extremo a velocidade e automação:
    *   **Bônus Especiais de Conjunto**:
...
### E. Slots de Amuleto (v7.0.0, reformulado na v12.0.0), Anel (v8.0.0, reformulado na v12.0.0) e Relíquia Ativa (v9.0.0)
*   **Amuleto (`amulet`, v7.0.0 "Ecos que Despertam"; rework v12.0.0 "O Oráculo Rúnico")**: Sétimo slot de equipamento. Originalmente rolava **exatamente 1 bônus passivo** da mesma pool utilitária do Colar (Seção 5.D) — desde a v12.0.0, **não rola mais stats intrínsecos** (`itemStats = {}`): o item virou uma "chave" que habilita a tela do Oráculo Rúnico (nova 13ª construção da Cidadela Astral, Seção 17.L), onde todo o seu valor vem de Runas/Palavras Rúnicas Astrais engastadas num círculo de até 6 espaços. Drop com taxa **fixa de $8\%$** mantida sem alteração, sem influência da Sorte, sorteada separadamente da rolagem de equipamento normal. **Bloqueado de fusão na Forja** desde a v12.0.0 (ver Seção 17.L) — fundi-lo destruiria as Runas Astrais engastadas, que a fusão não preserva.
*   **Anel (`ring`, v8.0.0 "O Espelho Faminto"; rework v12.0.0)**: Oitavo slot de equipamento, classificado como **slot "pesado" de itemização normal** — participa da rolagem de drop escalada por Sorte (sem taxa fixa dedicada) e de `setName`/Bônus de Conjunto/Fusão Mística. Até a v11.x, recebia atributos primários **restritos à classe ativa** (`possibleStatsMap`, mesmo pool de Cabeça/Peito/Pernas/Luvas/Arma). Desde a v12.0.0, `StatEngine.generateRingStats` rola **1 dos 6 atributos primários (Força, Magia, Destreza, Constituição, Sorte, Toque) independente da classe**, com o valor no **dobro** da escala normal de peças pesadas (compensando ser só 1 stat, em vez de 1-3 escolhidos do pool restrito). A Fusão Mística passou a exigir que os dois anéis tenham **o mesmo atributo** — não é possível fundir um anel de Força com um de Sorte, por exemplo.
*   **Relíquia Ativa (`activeRelic`, v9.0.0 "O Que Espera no Pandemônio")**: Nono slot, categoria própria — nem "leve" (sem stats de `BaseStats`) nem "pesado" (não participa de `setName`/Fusão Mística). Concede uma habilidade ativa com recarga (ver Histórico de Updates e Otimizações de Engenharia.md, entrada da v9.0.0, e Seção 7 para o fluxo de `skillCooldowns` reaproveitado). Drop independente, fixo em $2\%$ por abate fora da Torre; a raridade (`common`/`rare`/`legendary`) só afeta o range de roll do parâmetro da habilidade, nunca stats de atributo. Desde a v12.0.0, o mesmo mecanismo de buff (`skillCooldowns` + flags `isActiveRelic*Active`) passou a ser reaproveitado também pelas Palavras Rúnicas Astrais lendárias do Amuleto (Seção 17.L).
*   **Contagem de slots equipáveis ao longo das versões**: 5 (base) → 6 com o Colar (v5.0.0) → 7 com o Amuleto (v7.0.0) → 8 com o Anel (v8.0.0) → 9 com a Relíquia Ativa (v9.0.0). A contagem permanece em 9 na v12.0.0 (o Oráculo Rúnico é uma construção da Cidadela, não um slot novo). A desmontagem de equipamentos (Seção 5.C) e o desbloqueio de bônus de 2/3/5 peças (Seção 5.B) não se aplicam à Relíquia Ativa (fora do sistema de sets).
...
*   **Recompensa de Progresso**: Ao derrotar inimigos e superar andares na Torre Infinita, o jogador é recompensado com uma nova moeda especial chamada **Fragmentos de Forja** (`forgeFragments`). **A partir da v12.1.0 "Oficina Reforjada"**, a Torre é a **única fonte** de Fragmentos do jogo — a Oficina de Automação da Forja da Cidadela deixou de produzi-los, passando a produzir Cristal Rúnico (ver Seção 17.F).
...
### I. Títulos Honoríficos com Bônus de Stat (v11.5.0 "Títulos com Propósito")
Até a v11.4.0, os 3 pools de títulos honoríficos do jogo (Torre Normal, Ramificação de Maldições e Profundezas do Abismo — Seção 18.B) eram puramente estéticos. A v11.5.0 dá função a eles, transformando a escolha de título em uma decisão estratégica.
*   **Um Único Título Ativo, Compartilhado entre as 3 Listas**: `equippedTitle` (`useTowerStore.ts`) já era um campo único usado tanto pela Torre (Normal/Maldições) quanto pelas Profundezas (`selectTitle()` sobrescreve o mesmo campo não importa a lista de origem) — a v11.5.0 aproveita essa estrutura já existente para a nova regra central: **apenas o título atualmente equipado concede seu bônus**; os bônus não são cumulativos entre as 3 listas, então trocar de título troca o bônus ativo.
*   **Bônus por Lista, Escalando com o Nível do Título**: o "nível" de um título é sua posição (1ª a 6ª) dentro da lista de 6 marcos daquela ramificação/modo:
    *   **Torre Normal** (`NORMAL_TITLE_MILESTONES`): **+2% de Vida Máxima por nível** (`maxHpPct`), de +2% (Iniciante da Torre) a +12% (Lenda Eterna, nível 6).
    *   **Ramificação de Maldições** (`CURSE_TITLE_MILESTONES`): **+2% de Dano Geral por nível** (`damageMultiplierPct`), de +2% a +12%.
    *   **Profundezas do Abismo** (`PROFUNDEZAS_TITLE_MILESTONES`): **+5% de Dano Crítico por nível** (`critDamage`, valor direto em pontos percentuais — mesma unidade da pesquisa `researchCritDmgLevel` da Academia), de +5% a +30%.
*   **Implementação**: novo módulo `src/core/titleFormulas.ts` centraliza os 3 `Record<number, string>` de marcos (movidos de `useTowerStore.ts`/`abyssFormulas.ts`, que passaram a só reexportá-los) e a função `getEquippedTitleBonus(equippedTitle)`, que identifica a qual lista o título pertence e retorna o bônus correspondente. `StatEngine.calculateFinalStats` lê `useTowerStore.getState().equippedTitle` e soma o resultado em `finalStats.maxHpPct`/`damageMultiplierPct`/`critDamage`, no mesmo padrão aditivo já usado pelas pesquisas da Academia (Seção 17.F).
*   **Interface**: a galeria de títulos (`TowerPanel.tsx` e `AbyssPanel.tsx`) exibe o bônus de cada título já no card, e o `EquippedTitleBox` (componente compartilhado pelas 3 listas) mostra o bônus concedido pelo título atualmente equipado, junto de um aviso de que o bônus não acumula entre listas.
...
*   **Condições de Desbloqueio do Rito** (`performTranscendence`, `useGameStore.ts`): as quatro precisam ser verdadeiras simultaneamente, e o herói não pode estar dentro da Torre Infinita nem do Desafio Diário no momento:
    ...
    4.  **(v12.1.0) Gate de Crescimento Mínimo**: o PP vitalício usado nesta Transcendência (`totalPP`, mesmo valor da fórmula de conversão abaixo) precisa ser **pelo menos 25% maior** que o PP usado na Transcendência anterior (`lastTranscendencePPUsed`, novo campo persistido em `Character`). Na primeira Transcendência da conta, `lastTranscendencePPUsed` é 0/ausente e o gate é automaticamente satisfeito. Esse requisito existe para impedir que o jogador abuse do bônus permanente por ciclo (item abaixo) transcendendo repetidamente com pouquíssimo progresso adicional entre os ritos — a UI (`TranscendencePanel`, `GameUI.tsx`) exibe o PP mínimo necessário para a próxima Transcendência e bloqueia o botão com uma mensagem explícita quando o gate não é atingido.
...
*   **O que é mantido**: os **Pontos de Transcendência** acumulados (somam com o novo ganho), a **árvore de Talentos de Transcendência** já comprada, o contador `transcendenceCount`, o novo `lastTranscendencePPUsed` (atualizado para o `totalPP` desta Transcendência, usado no gate do próximo ciclo), e as construções da Cidadela (exceto o conteúdo do Depósito). **Cristal Rúnico** (`runicCrystals`, v12.1.0 — ver Seção 17.F) é zerado neste reset, seguindo o mesmo tratamento já dado a Fragmentos de Forja.
...
*   **Bônus Permanente por Ciclo (v12.1.0: 5% → 20%)**: `StatEngine.getTranscendenceBoost(character) = 1 + transcendenceCount × 0.20` — cada Transcendência concede **+20%** multiplicativo permanente de Dano/Vida Máxima/Mana Máxima, empilhando indefinidamente por ciclo (2 ciclos = +40%, 3 = +60%...). Aplicado **por fora** do pool aditivo de `damageMultiplierPct`/`maxHpPct`/`maxManaPct` (mesmo espírito de `alma_avatar`/da Runa Primordial Ecoh), nas 3 cadeias de `setDamageMultiplier` e no `hpBoost`/`manaBoost` do `CombatFSM.ts` — garante que o bônus continue valendo mesmo depois do reset de equipamento e Ascensões causado pela própria Transcendência. O valor original (5%) foi considerado baixo demais frente à perda de progresso que o Rito impõe; o salto para 20% é balanceado pelo gate de crescimento mínimo do item B.4 acima, que impede repetir o Rito sem progresso real entre ciclos.

### C. A Zona Espelho: Ecoterra (debuffs reforçados na v12.1.0)
...
*   **Modificadores e Status dos Inimigos (v12.1.0: valores aumentados)**: Para representar a distorção temporal do ciclo espelhado, todos os monstros gerados na Ecoterra (Fases 1–20) recebem buffs de status significativos:
    *   **HP Máximo**: Aumentado em **$+50\%$** de forma multiplicativa (antes $+30\%$).
    *   **Velocidade de Ataque**: Aumentada em **$+35\%$** (antes $+20\%$), reduzindo o tempo de recarga base do oponente — desde a v12.1.0, mitigável pelo Sifão Cósmico (item abaixo), penalidade que antes não tinha nenhuma forma de mitigação.
*   **Penalidades Ambientais sobre o Jogador (Instabilidade da Alma, v12.1.0: valores aumentados + novo debuff)**:
    Enquanto o herói estiver combatendo na Ecoterra, ele sofre com a instabilidade da fresta temporal:
    *   *Drenagem de Mana*: Perda contínua de **$2.5\%$** da Mana Máxima do jogador por segundo (antes $1.5\%$).
    *   *Erosão Temporal*: O tempo de recarga (cooldown) de todas as habilidades ativas é aumentado em **$+25\%$** (antes $+15\%$).
    *   *Instabilidade Corrosiva (novo, v12.1.0)*: O jogador recebe **$+25\%$** de dano de todos os ataques inimigos enquanto a Ecoterra está ativa — o debuff que introduz risco real de morte no modo, ausente antes desta versão. Calculado dentro do ramo de dano da campanha normal em `performEnemyAttack` (`CombatFSM.ts`), aplicado depois do dano base do inimigo e antes das reduções de defesa do jogador (Constituição, Escudo Ósseo, `damageReductionPct`, runas Dol).
    *   Todas as quatro penalidades (mana, erosão, velocidade de ataque e dano recebido) são mitigáveis pelo **Sifão de Essência Cósmica** da Cidadela — ver Seção 17.G — de forma linear por nível, chegando a neutralização total no Nível 5 ("Sincronia Perfeita"). É a válvula de escape oficial para quem achar a Ecoterra difícil demais: desativar o switch ou investir no Sifão.
...
*   **Recompensa Adicional: Carga Cósmica (v12.1.0)**: combater na Ecoterra também acumula **Carga Cósmica** (`citadel.cosmicSiphon.cosmicCharge`, teto 100) a uma taxa de `2 + nível_do_Sifão` por segundo de combate — alimenta a habilidade ativa **Pulso Cósmico** do Sifão de Essência Cósmica (Seção 17.G), dando à Ecoterra um segundo propósito de farm além da ET.
...
*   **Itens Não-Elegíveis**: Consumíveis, Relíquias Ativas (`activeRelic`, Seção 5.E) e, desde a v12.0.0, Amuletos (`amulet`, Seção 5.E/17.L) nunca são elegíveis para fusão — nem aparecem como opção de seleção no altar, nem passam pela validação de `reforgeItems`. Relíquias Ativas foram adicionadas a essa exclusão como correção pós-lançamento da v9.0.0 (antes, duas relíquias do mesmo slot podiam ser fundidas indevidamente, já que `activeRelic` é tratado como um `slot` de equipamento comum na validação de compatibilidade); Amuletos foram adicionados na v12.0.0 pelo mesmo motivo — a fusão não preserva os campos `amuletSockets`/`activeAstralRunewords` (só `sockets`/`socketedRunes` do sistema de soquetes pesado são copiados para o item resultante), então fundi-los destruiria silenciosamente as Runas Astrais engastadas.
*   **Anéis exigem o mesmo atributo (v12.0.0)**: além das condições gerais acima, dois Anéis (`ring`) só são elegíveis para fusão se rolaram o **mesmo atributo primário** (ex.: dois anéis de Força) — não é possível fundir um anel de Força com um de Sorte, mesmo que sejam do mesmo conjunto e raridade.
...
*   **(v12.1.0) Cristal Rúnico a partir de +3**: fusões partindo de Místico **+3 em diante** (indo para +4, +5, +6, +7 ou +8) passam a exigir também **Cristal Rúnico** (`runicCrystals`, moeda produzida pela Oficina da Cidadela — ver Seção 17.F), além do Ouro e Fragmentos já cobrados (valores desses dois **inalterados**). `getMysticFusionCost` (`citadelFormulas.ts`) retorna um `crystalCost` de 0 para os níveis +1/+2/+3 (sem exigência) e escalando a partir daí (tabela abaixo).

| Nível de Origem | Nível Resultante | Custo em Ouro | Custo em Fragmentos de Forja | Custo em Cristal Rúnico (v12.1.0) |
| :--- | :--- | :--- | :--- | :--- |
...
| Místico +3 + Místico +3 | Místico +4 | $12.500$ Ouro | $2.500$ Fragmentos | $50$ Cristais |
| Místico +4 + Místico +4 | Místico +5 | $62.500$ Ouro | $6.250$ Fragmentos | $150$ Cristais |
| Místico +5 + Místico +5 | Místico +6 | $312.500$ Ouro | $12.500$ Fragmentos | $400$ Cristais |
| Místico +6 + Místico +6 | Místico +7 | $1.562.500$ Ouro | $25.000$ Fragmentos | $1.000$ Cristais |
| Místico +7 + Místico +7 | Místico +8 | $7.812.500$ Ouro | $50.000$ Fragmentos | $2.500$ Cristais |
...
interface CitadelState {
  ...
  cosmicSiphon: CitadelBuildingState & { cosmicCharge: number }; // v12.1.0: Carga Cósmica, ver Seção 17.G
...
runicCrystals?: number; // v12.1.0: Cristal Rúnico — moeda da Oficina, ver Seção 17.F
```
...
### F. Torre de Vigia Astral e Oficina de Automação da Forja (v5.3.0, Oficina reformulada na v12.1.0)
...
*   **Produção passiva de Cristal Rúnico (v12.1.0, antes produzia Fragmentos de Forja)**: a Oficina produzia Fragmentos de Forja, mas a Torre Infinita já concede o mesmo recurso em quantidades ordens de magnitude maiores (Seção 10.E), tornando a estrutura irrelevante em qualquer estágio pós-early game. Reformulada para produzir **Cristal Rúnico** (`runicCrystals`), a nova moeda de endgame da Forja Mística (fusões +3 em diante, Seção 14.B) e das duas novas funções ativas abaixo — Fragmentos de Forja passaram a ser exclusivos da Torre.
*   Converte Ouro e Madeira excedentes em Cristal Rúnico através de "ordens de serviço" passivas de 1 hora (50.000 Ouro + 50 Madeira → **5 Cristais** por ordem, `FORGE_ORDER_CRYSTAL_YIELD`), com o nível da Oficina determinando quantas ordens paralelas podem ser processadas por hora. A resolução das ordens concluídas acontece dentro de `tickCitadelProduction()` (`useGameStore.ts`), que avança `forgeWorkshop.lastTick` apenas pelas **horas inteiras já processadas** (`lastTick += horasInteiras × 3600000`) em vez de saltar direto para `Date.now()` — preserva a fração de hora ainda em andamento entre chamadas consecutivas (a própria `GameUI.tsx` chama essa função a cada 60 segundos com o jogo aberto), garantindo que uma ordem complete assim que sua hora se esgota, tanto em sessão contínua quanto após um período offline.
*   **Nível 5 "Mestre Forjador"** desbloqueia o **Desmonte Automatizado**: equipamentos de raridade Comum ou Rara "puros" (sem pertencer a um conjunto Ancestral, Pandemoníaco ou Celestial) dropados em combate são convertidos instantaneamente em +1 Fragmento de Forja em segundo plano, sem nunca ocupar um slot do inventário (`CombatFSM.ts`, fluxo de drop de equipamento) — este perk continua gerando Fragmentos normalmente, só a produção passiva por hora mudou para Cristal Rúnico.
...
*   **Função Ativa 1 (v12.1.0) — Reroll de Atributos** (`rerollItemStats`, `useGameStore.ts`): permite selecionar qualquer item equipável do inventário (exceto Consumíveis, Relíquia Ativa e Amuleto) e rerolar seus atributos por Cristal Rúnico. A cada reroll é sorteada uma **fase efetiva** no intervalo `[max(1, currentStage − 5), currentStage + 5]` — os atributos do item são recalculados como se ele tivesse sido gerado nessa fase (mesma fórmula de escala usada na geração normal de drops, `val = max(1, round(faseEfetiva × mult × (0.8 + random × 0.4)))`, com o mesmo teto por stat percentual usado na Fusão Mística), então o resultado pode ficar **melhor ou pior** que o item atual — é uma aposta, não uma melhoria garantida. **Custo**: `100 + 100 × mysticLevel` Cristal Rúnico por tentativa (itens não-místicos usam `mysticLevel = 0`).
*   **Função Ativa 2 (v12.1.0) — Melhoria/Destruição** (`improveOrDestroyItem`, `useGameStore.ts`): ação de risco sobre um item selecionado, com três resultados de probabilidade igual (1/3 cada): **Melhoria Lendária** (todos os atributos do item multiplicados por 1.5×, arredondado para cima — mesmo multiplicador da Forja Lendária, Seção 14.C), **nada acontece** (Cristal Rúnico consumido sem efeito) ou **destruição** (o item é removido permanentemente do inventário/equipamento). **Limite de segurança**: só **1 tentativa por nível de fusão mística** do item (`EquipmentItem.forgeAttemptUsedAtLevel`) — como cada Fusão Mística já gera um objeto de item novo (Seção 14), o contador nasce implicitamente "zerado" a cada novo nível, então a única forma de tentar de novo é fundir o item para o próximo nível místico. **Custo**: `200 + 200 × mysticLevel` Cristal Rúnico por tentativa.

### G. Sistemas de Fim de Jogo: Sifão, Altar e Laboratório (v5.4.0, Sifão reformulado na v12.1.0)
**Sifão de Essência Cósmica** — Custo base 1500 Pedra / 1000 Madeira / 50 Essências de Transcendência:
*   **Mitigação total das quatro penalidades da Ecoterra (v12.1.0, ver Seção 11.C)**, de forma linear por nível — antes o Sifão só mitigava 2 das 3 penalidades existentes (a velocidade de ataque inimiga não era afetada, e o novo debuff de dano recebido não existia):
    *   Drenagem de mana: `max(0, 2.5% - nível × 0.5%)` (base aumentada de 1.5% para 2.5%).
    *   Erosão de recarga: `max(0, 25% - nível × 5%)` (base aumentada de 15% para 25%).
    *   Velocidade de ataque inimiga: `max(0, 35% - nível × 7%)` — **nova mitigação (v12.1.0)**; antes essa penalidade não era afetada pelo Sifão em nenhum nível.
    *   Dano recebido (novo debuff, Seção 11.C): `max(0, 25% - nível × 5%)`.
*   No **Nível 5 "Sincronia Perfeita"**, as quatro penalidades são completamente neutralizadas, permitindo lutar na Ecoterra com 100% da capacidade técnica original.
*   **Bônus Ofensivo Próprio (v12.1.0)**: além de só mitigar penalidades, o Sifão agora concede um bônus positivo de dano ao jogador enquanto a Ecoterra está ativa — `+3% de Dano por nível` (`getCosmicSiphonOffensiveMult`, `CombatFSM.ts`), chegando a **+15%** no Nível 5, empilhado multiplicativamente com os demais multiplicadores de dano do jogo (mesma cadeia de `setDamageMultiplier` usada pelo bônus permanente de Transcendência).
*   **Habilidade Ativa "Pulso Cósmico" (v12.1.0)**: o Sifão acumula **Carga Cósmica** (`citadel.cosmicSiphon.cosmicCharge`, teto 100) a `2 + nível` por segundo de combate na Ecoterra (Seção 11.C). Ao encher, um botão dedicado no painel de Habilidades Ativas (visível só durante a Ecoterra) permite ativar `triggerCosmicSiphon()` (`CombatFSM.ts`, disparado via `GameEvent.TRIGGER_COSMIC_SIPHON`), consumindo toda a carga e concedendo o buff **"Sincronia Cósmica"**: `+50% + 5%×nível` de Dano e **Invulnerabilidade Total** (imune a todo ataque inimigo, mesmo mecanismo de bloqueio já usado pelo Elixir do Defensor e pelas Relíquias Ativas de invulnerabilidade) por **10s + 2s por nível do Sifão** (14s a 20s, dependendo do nível). Segue o mesmo padrão arquitetural das Relíquias Ativas (flags de buff temporário decrementadas por tick, entrada na bandeja de buffs ativos do HUD via `ACTIVE_BUFFS_CHANGED`), mas sem cooldown — limitado apenas pela taxa de acúmulo de Carga.
...
### L. Oráculo Rúnico e o Sistema de Runas/Palavras Rúnicas Astrais (v12.0.0)
O Amuleto (Seção 5.E) foi implementado na v7.0.0 com a intenção de ser um item de entrada simples, mas acabou reaproveitando o mesmo padrão raso do Anel (1 stat aleatório de um pool minúsculo). A v12.0.0 dá a ele uma tela própria e o sistema de maior profundidade do jogo, ancorado numa 13ª construção da Cidadela Astral.

*   **Décima Terceira Construção da Cidadela**: `amuletOracle: CitadelBuildingState & { selectedBuffKey?: 'dropChancePct' | 'critChance' | 'lifesteal' }` (`citadelFormulas.ts`/`useGameStore.ts`), sub-aba própria (`CitadelTabsBar.tsx`, ícone 🔮) e sprite de evolução 2×2 (`citadel_runicOracle.png`). Custo de melhoria em Madeira/Pedra/Pérolas Abissais, escala exponencial (`AMULET_ORACLE_UPGRADE_COST`, `×1.7`/`×1.8`), 5 níveis máximos.
*   **O Amuleto vira uma "chave"**: desde a v12.0.0, `generateAmuletStats` foi removido e o Amuleto **não rola mais stats intrínsecos** (`itemStats = {}` em `CombatFSM.ts`/`sunkenCitadelFormulas.ts`) — seu valor vem inteiramente das Runas Astrais engastadas nele, geridas na tela do Oráculo. Sem o Oráculo construído, um amuleto equipado não tem efeito algum.
*   **Círculo de 6 espaços e dois modos de uso** (`AmuletOraclePanel.tsx`, `astralRuneFormulas.ts`): o Oráculo libera progressivamente até 6 espaços de runa dispostos em círculo ao redor do sprite do amuleto (N1: 3 · N2: 4 · N3: 5 · N4+: 6, `getMaxAmuletSlots`), mais 1 espaço central que exibe a palavra reconhecida. Os espaços suportam dois modos **mutuamente exclusivos**, testados por `getActiveAstralRunewords`:
    1.  **Palavra única**, comprimento 3 a 6, ocupando os espaços `0..N-1` — tem prioridade se encontrada.
    2.  **Duas palavras diferentes de 3 runas cada**, uma em cada metade do círculo (espaços 0-2 e 3-5), ativas **simultaneamente** — só possível com os 6 espaços desbloqueados (Oráculo N4+).
    A detecção é **passiva**: o jogador engasta livremente as runas (sem custo de gravação) e clica em "🔮 Consultar o Oráculo" (`activateAstralRuneword`), que testa a sequência atual contra o catálogo sem consumir nenhuma runa — diferente das Palavras Rúnicas do sistema de soquetes pesado (Seção 18.L), que exigem uma ação de gravação explícita que consome as runas do cofre. Runas soltas nunca dão bônus fora de uma palavra reconhecida.
*   **Catálogo próprio, independente do sistema pesado**: `astralRuneFormulas.ts` define 9 Runas Astrais (3 tiers, tema suporte/utilidade/habilidade — Eco da Regeneração, Passo Leve, Olho Astral, Maré Psíquica, Chama Interior, Véu Sombrio, Coroa Estelar, Pulsar do Vazio, Graal do Oráculo) e 6 Palavras Rúnicas Astrais, deliberadamente **não** reaproveitando `RUNE_CATALOG`/`RUNEWORD_CATALOG` de `runeFormulas.ts`. Fontes de obtenção (`CombatFSM.ts`): Tier 1-2 em conteúdo endgame (pós-1ª Ascensão, drop de **0,3% por abate** — reduzido de 3% após playtest indicar acúmulo rápido demais para um material de uso único) e produção passiva do Oráculo a partir do Nível 3 (1 runa a cada 6h de tempo real, mesmo padrão de acúmulo offline da Oficina da Forja); Tier 3 exclusivo de **Torre nível 100+** ou **Pandemônio (Fase 50+)**, **0,15% de chance por abate** (reduzido de 2%).
*   **Lista de Palavras Rúnicas Astrais conhecidas + revelação em 2 métodos**: como não existe nenhum indicador em jogo de quais sequências formam uma Palavra Rúnica Astral, `Character.revealedAstralRunewordIds` rastreia as já descobertas (mesmo padrão do `revealedRunewordIds` do sistema pesado). Revelação por **(1) tentativa e erro**: toda palavra reconhecida com sucesso ao clicar em "Consultar o Oráculo" (`activateAstralRuneword`) entra na lista permanentemente, mesmo que as runas sejam trocadas depois; ou **(2) Garrafa Perdida** (ver bullet abaixo). `AmuletOraclePanel.tsx` exibe a lista completa logo abaixo do picker de runas — receitas não reveladas mostram `"??? — uma sequência desconhecida ressoa..."`; reveladas mostram a sequência completa (glifos), nome, efeito e nível mínimo do Oráculo. É só informativa — diferente da Câmara de Gravação, não há botão de aplicar automaticamente; o jogador monta a sequência manualmente e na ordem certa no círculo.
*   **🍾 Garrafa Perdida (novo consumível)**: item `consumableType: 'garrafa_perdida'`, drop de **5% por puxada** de "Puxar a Linha" (Pesca Ativa do Litoral, Seção 18.A) — 1 rolagem por puxada, independente do resultado (erro/acerto/perfeito), então acerto/perfeito não dobra a chance (`resolveActiveFishing`, `useGameStore.ts`). Ao consumir (`useConsumable`): 5% de chance de revelar 1 Palavra Rúnica Astral aleatória ainda não descoberta (removida do pool de sorteio ao ser revelada — nunca sai repetida); se todas as 6 já estiverem reveladas, cai automaticamente no prêmio de material (nunca desperdiça a garrafa); os outros 95% dão **+5 Pérolas Abissais ou +10 Coral Vivo** (escolha 50/50).
*   **Palavras lendárias concedem habilidades ativas novas**: as 2 receitas de comprimento 6 (`grantsActiveAbility`, liberadas só no Oráculo N5) reaproveitam o mesmo mecanismo de buff já usado pela Relíquia Ativa (Seção 5.E) em vez de um sistema paralelo — `CombatFSM.triggerAmuletAbility()` usa os mesmos flags de buff (`isActiveRelicDamageBuffActive`/`isActiveRelicHealBuffActive`, etc.) e o mesmo mapa `skillCooldowns` (chave `amulet_ability_${runewordId}`), disparado por um novo `GameEvent.TRIGGER_AMULET_ABILITY` e um botão dedicado no HUD de combate (`ActiveSkillsPanel`, ao lado do botão da Relíquia Ativa).
*   **Bônus fixo e permanente da ESTRUTURA (não do item)**: além do sistema de runas, o Oráculo sorteia — **1 vez, na 1ª construção** — 1 stat do antigo pool que o Amuleto dava antes do rework (`AMULET_ORACLE_BUFF_POOL`: `dropChancePct`/`critChance`/`lifesteal`), gravado em `selectedBuffKey`. Nos upgrades seguintes o mesmo stat só cresce em magnitude (`getAmuletOracleBuffValue`, escala linear de 20% a 100% do teto entre os níveis 1 e 5, sem re-sortear) e é aplicado em `StatEngine.calculateFinalStats` (passo 4.9) **independente de o jogador ter algum amuleto equipado** — é um bônus da construção, não do item. Um botão "🎲 Rerolar Bônus" (`rerollAmuletOracleBuff`) permite trocar o stat sorteado por um custo alto e escalando com o nível (`AMULET_ORACLE_REROLL_COST`: Ouro + Pérolas Abissais), sempre sorteando um stat **diferente** do atual.
*   **Gerencia qualquer amuleto, não só o equipado**: `socketAstralRune`/`unsocketAstralRune`/`activateAstralRuneword` (`useGameStore.ts`) operam sobre um `itemId` via `updateItemEverywhere` (mesmo helper reaproveitado pela Câmara de Gravação) — o painel oferece uma "Vista 1" de seleção de amuleto entre equipado, inventário e Depósito da Cidadela, permitindo remontar as runas de um amuleto novo sem precisar reequipar um antigo só para resgatar as runas dele primeiro.
*   **Bloqueio de fusão na Forja (correção)**: como todo o valor do Amuleto vive em `amuletSockets`/`activeAstralRunewords` — campos que a fusão de `reforgeItems` não preserva no item resultante (só `sockets`/`socketedRunes` do sistema pesado são copiados) —, o Amuleto foi bloqueado de fusão na Grande Forja Arcana (`ForgeView.tsx`/`reforgeItems`), mesmo tratamento já dado ao slot `activeRelic`: fundi-lo destruiria silenciosamente as Runas Astrais engastadas.
...
Novo teto de multiplicador de status, **8.0×** (acima do Celestial, 7.0×) — rolagem inteiramente separada da cadeia de exclusividade da campanha (Ancestral/Lua de Sangue/Pandemônio/Celestial em `CombatFSM.handleEnemyDefeat`), já que as Profundezas nunca usam esse pipeline. Só dropa na Fossa do Caco (Zona 4, 4% por abate, `rollAbyssalSetDrop`/`ABYSSAL_SET_DROP_CHANCE`) e é garantido em toda morte do Leviatã.

**Bônus de conjunto (`StatEngine.ts`, reforçado v-next)**: por ter o maior multiplicador de atributos do jogo, o Abissal deveria ser o set mais forte na prática — o que não acontecia com os valores anteriores (+30% Dano/+12% Vida no 5pc, abaixo do Celestial, e nenhum bônus de status no 3pc). Bônus de 3 peças: **+1 soquete de runa** acima do teto normal (até 4 na arma, mantido) **+6% de Roubo de Vida** (novo, acima do 3pc de qualquer outro set de topo). Bônus de 5 peças: **+65% Dano Final, +30% Vida Máx., +12% Vel. Ataque** (os 3 acima da progressão do Celestial: +40%/+20%/+10%) e **+10% de Chance Crítica** (mecânica exclusiva do Abissal — nenhum outro set de topo concede crítico), além da imunidade a [ENCHARCADO] já existente.
...
## 19. Modo História "Ecos do Destino" e Diário de Jornada (v11.0.0, revisado na v11.1.0, na v11.2.0, na v11.3.0 e na v11.4.0)
...
### K. Revelação de Artefato de História (`ArtifactRevealOverlay.tsx`) — v11.2.0, halo corrigido na v11.4.0
...
*   **Bug do halo "vazado" (corrigido na v11.4.0)**: o halo atrás do sprite (`<div>` circular de 260px com `background: radial-gradient(...)`) era animado via `@keyframes artifact-reveal-glow` que alterava **`box-shadow`**, não `opacity`/`background`. Como `box-shadow` é desenhado a partir da borda do elemento pra fora, num círculo isso pinta um anel de luz forte começando exatamente na borda (130px de raio) e se espalhando pra fora dela — enquanto o preenchimento real do gradiente já esmaecia para transparente bem antes disso (a 58% do raio, ~75px), deixando a região atrás do sprite sem brilho nenhum. Não era um bug de `overflow`/`clip-path` (nenhum dos dois está presente) nem de stacking (o `<img>` já ficava corretamente por cima do halo). Corrigido seguindo o mesmo princípio já usado por `.alma-mundo-flame-layer`/`@keyframes alma-mundo-flicker` (chama da Voz da Alma-Mundo, Seção 3.G): a keyframe passou a animar `opacity` + `transform: scale(...)` (mantendo o `translate(-50%,-50%)` de centralização do elemento, já que animar só `scale` sobrescreveria o `transform` inline e quebraria a centralização), e o gradiente do halo passou a se estender até 85% do raio (em vez de 58%) com `filter: blur(8px)` e `mixBlendMode: 'screen'`, dando um disco de luz suave atrás do sprite em vez de um anel vazado.
