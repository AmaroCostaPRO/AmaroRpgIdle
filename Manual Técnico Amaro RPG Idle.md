...
*   **Recompensa: Essência de Transcendência (ET)**:
    Monstros derrotados na Ecoterra concedem **Essência de Transcendência** (`transcendenceEssence`), um recurso de alta raridade utilizado na Loja Celestial. Valor por abate (`CombatFSM.ts`): `Math.max(1, Math.floor(base / 4))`, onde `base` é 5 para chefes, 2 para Elites e 1 para inimigos comuns — resultando em 1 ET por abate na prática (piso mínimo de 1), 4x menor que a taxa original.
...
*   **Preparo Manual, Não Produção por Tick**: diferente da Oficina de Automação da Forja (ordens de serviço horárias automáticas), o Laboratório usa a ação `brewAlchemyPotion(potionType)` — preparo sob demanda que consome Madeira/Pedra/Carne na hora e entrega o rendimento (1 a 3 poções, escalando com o nível do laboratório) direto no inventário como itens consumíveis.
...
*   **Rotação e recompensas**: 2 contratos ativos a partir do Nível 1, 3 a partir do Nível 3 (`HUNT_CONTRACT_SLOTS`); cada um pede a morte de N unidades de um inimigo sorteado de um pool de 28 ids (`HUNT_CONTRACT_ENEMY_POOL`, duplicado de `ENEMY_TYPES` para evitar import circular entre `CombatFSM.ts` e `citadelFormulas.ts`). Recompensa em material+ouro escala com o nível do Santuário e a dificuldade do alvo; completar toda a rotação concede Fragmentos de Alma Instável extras via `useRelicStore`.
...
### Versão 9.5.1 "Ajustes de Balanceamento e Correções" (Atual)
