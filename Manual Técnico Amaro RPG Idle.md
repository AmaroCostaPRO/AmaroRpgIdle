...
*   **Classes Primárias de Magia (Mago, Clérigo, Necromante)**:
    ...
    *   Regeneração de Mana ganha por ponto de Magia: $0.10\text{ Mana/s}$ *(ajuste pós-lançamento da v9.0.0 — antes $0.02\text{ Mana/s}$; ver Seção 16)*
*   **Outras Classes (Guerreiro, Arqueiro, Paladino, Ladrão)**:
    ...
    *   Regeneração de Mana ganha por ponto de Magia: $0.20\text{ Mana/s}$ *(ajuste pós-lançamento da v9.0.0 — antes $0.05\text{ Mana/s}$; ver Seção 16)*
...
Para otimizar o gerenciamento do inventário de 30 slots, o jogador pode aplicar uma ação a **todos** os equipamentos do inventário de uma vez através de dois botões integrados ao final do painel de inventário — ambos operam sobre qualquer raridade/tipo de equipamento (Comum a Místico), sem segmentação por raridade (ajuste da v9.0.0; ver Seção 16, changelog da v9.0.0, para o desenho anterior segmentado):
...
3.  **Aba de Runas** (`RuneInventoryPanel.tsx`, v10.5.0): mostruário **somente leitura** do cofre de runas (`character.runeInventory`) — mostra cada runa possuída (ícone + quantidade via `RuneChip`) sem nenhuma ação de venda, uso ou equipagem. Clicar numa runa abre um modal local (mesmo padrão de posicionamento da Seção 3.D) com nome, tier/Primordial e descrição do efeito (`describeRuneEffect`, função compartilhada com a Câmara de Gravação, ver Seção 19.G).
...
interface CitadelState {
  ...
  alchemyLab: CitadelBuildingState; // v8.0.0 "O Espelho Faminto" — ver Seção 18.J
}
...
*   Converte Ouro e Madeira excedentes em **Fragmentos de Forja** através de "ordens de serviço" passivas de 1 hora (50.000 Ouro + 50 Madeira → 15 Fragmentos por ordem, custo em Ouro elevado de 200 para 50.000 na v9.1.0), com o nível da Oficina determinando quantas ordens paralelas podem ser processadas por hora.
...
*   **🐛 Correção: Ordens Nunca Completavam Durante Sessão Ativa (v9.7.0)**: `tickCitadelProduction()` (`useGameStore.ts`) resetava `forgeWorkshop.lastTick` para `Date.now()` a cada chamada, mesmo quando `elapsedHours` não somava uma hora inteira e nenhuma ordem havia sido processada. Como a própria `GameUI.tsx` chama `tickCitadelProduction()` a cada 60 segundos enquanto o jogo está aberto, esse reset descartava o progresso parcial antes de completar 1h — na prática, a Oficina só produzia Fragmentos quando o jogo era reaberto após ficar fechado por 1h+, nunca durante uma sessão contínua. Corrigido para só avançar `lastTick` pelas **horas inteiras já processadas** (`lastTick += horasInteiras × 3600000`), preservando a fração restante para o tick seguinte.
*   **📊 Barra de Progresso da Ordem de Serviço (v9.7.0)**: `ForgeWorkshopPanel.tsx` ganhou um indicador visual do ciclo de 1h em andamento, via o novo hook `useForgeOrderProgress` (`src/hooks/useForgeOrderProgress.ts`) — reativo a cada segundo (mesmo padrão de `useCountdown`, porém calculando o resto da divisão do tempo decorrido pelo tamanho do ciclo, já que a ordem se repete indefinidamente em vez de ter um único `completesAt`). Exibe percentual preenchido e tempo restante até a próxima ordem; se Ouro ou Madeira forem insuficientes para a próxima ordem, o painel troca a barra por um aviso de "produção pausada" em vez de sugerir um progresso que não vai se concretizar.
...
*   **Décima Construção da Cidadela**: `alchemyLab: CitadelBuildingState`, seguindo o mesmo modelo de dados/upgrade em tempo real das demais 9 construções (Seção 18.B/I) — custo em Madeira/Carne/Insígnias de Estudo, gated pelo nível do Centro de Comando.
...
*   **Décima Primeira Construção da Cidadela**: `huntSanctuary: CitadelBuildingState & { activeContracts: HuntContract[]; rotationId: number; bonusClaimedForRotation: boolean }`, ocupando a 2ª posição livre da 2ª página do pátio (ao lado do Laboratório de Alquimia, Seção 18.J) — `PAGE_2_SUB_TABS` em `CitadelTabsBar.tsx` estendido para incluir `'huntSanctuary'`.
...
## 19. A Cidadela Submersa (v10.0.0): Litoral, Profundezas e o Trono Afundado

Maior expansão de conteúdo do jogo, construída como um arco paralelo à campanha/Cidadela Astral: uma economia própria (Pérolas Abissais, Coral Vivo, Runas) que nunca contamina XP/Ouro/equipamento normal, acessada pela nova aba de topo 🌊 Abismo (`AbyssPanel.tsx`, três sub-abas: Litoral/Profundezas/Cidadela).

### A. O Litoral Naufragado (`CoastalPanel.tsx`, `abyssFormulas.ts`)
Desbloqueado ao completar a Fase 2 (`character.coastal.unlocked`). **Doca de Pesca** (`EvolutionSprite`, 5 níveis, `getCoastalDockUpgradeCost`): pesca passiva com buffer (`getFishingBufferCap` por nível) que pausa ao encher, coleta manual movendo o que couber no inventário (mesmo padrão "Coletar Chaves" da Torre de Vigia); pesca ativa via minigame de timing 100% React (janela verde de acerto, 20% central de acerto perfeito, sem nenhum envolvimento do Phaser). Três iscas craftáveis com Carne (`BAIT_DEFINITIONS`) mudam os pesos da tabela de captura (`getFishingTable`). Acertos perfeitos alimentam o contador vitalício da Runa Primordial **Faro** (`FARO_PERFECT_CATCHES_REQUIRED = 100`). Fragmentos de Batisfera pescados (5 por Chave, `BATHYSPHERE_FRAGMENTS_PER_KEY`) montam Chaves de Mergulho.

### B. As Profundezas — Mergulho Vertical Push-Your-Luck (`useDiveStore.ts`)
Modo inteiramente separado da campanha/Torre (exclusividade mútua enforçada em `CombatFSM.setupEnemyForLevel`: Torre > Leviatã > Mergulho > campanha). Gasta 1 Chave de Mergulho (2 nos checkpoints 26/51/81, `getDiveKeyCost`). **Fôlego** é o único relógio de sessão: `getBreathDrainPerSecond = 0.008 × (1 − 0.04×nível do Traje)` — deliberadamente **sem** termo de profundidade (corrigido na revisão pós-lançamento; a versão inicial multiplicava por `1+0.02×profundidade`, contradizendo o próprio design de "risco de sessão, não de poder"). Morte por Fôlego zerado ("Afogamento") aplica `DROWNING_DAMAGE_MULT = 2.0×` e mantém 50% do acumulado (`DIVE_DROWNED_KEEP_FRACTION`); morte comum mantém 75%; subir num Bolsão de Ar banca 100%. Bolsões de Ar a cada 5 profundidades (`AIR_POCKET_INTERVAL`) oferecem 3 escolhas (Fôlego/Runa da zona/Pérolas bônus), suspendendo o FSM como o encontro do Mercador.

**Zonas e Pressão**: `getZoneForDepth` (Zona 1: 1-25, Zona 2: 26-50, Zona 3: 51-80, Zona 4: 81+, infinita) multiplica HP/Dano dos inimigos por `ZONE_FACTORS` próprios. Pressão (`getPressureMultiplier = 1 + 0.04×profundidade × (1 − 0.06×nível do Traje)`) se aplica como redução de dano recebido do jogador, mitigada pelo Traje de Mergulho (10 níveis, `getDiveSuitUpgradeCost`, upgrade só disponível com a Doca Batial restaurada).

### C. Guardiões de Zona (`ZONE_GUARDIANS`)
3 chefes fixos nas profundidades 25/50/80 (Aracnídeo do Recife, A Coisa Entre as Algas, O Castelão Afundado), fora do pool de sorteio comum, com escudo periódico de 20% do HP (`GUARDIAN_SHIELD_PCT`, reconstrução a cada 15s sem escudo). O Guardião da Zona 1 (Aracnídeo do Recife, prof. 25) usa `hpMult: 3.0`/`dmgMult: 1.5` (revisão de balanceamento da v10.7.0 — antes `6.0`/`1.8`, junto com o Guardião de Zona 2/3); os Guardiões das Zonas 2 e 3 (A Coisa Entre as Algas, O Castelão Afundado) permanecem em `hpMult: 6.0`/`dmgMult: 1.8`, já que ainda não haviam sido reportados como gargalo de dificuldade. Cada 1ª morte garante uma Runa Primordial de zona (Thal/Vrak/Morvo). Vencer um Guardião libera o checkpoint da profundidade seguinte (Seção B). 6 títulos honoríficos (`PROFUNDEZAS_TITLE_MILESTONES`, marcos de recorde histórico de profundidade: 10/25/50/80/120/200), concedidos pela mesma infraestrutura de títulos da Torre Infinita (`useTowerStore.unlockTitle`).

**Revisão da curva de HP por profundidade (v10.7.0)**: o expoente de crescimento de HP dos inimigos comuns por profundidade (`getDiveEnemyHP`, `abyssFormulas.ts`) foi reduzido de `1.14` para `1.09` — reportado pelo usuário como uma parede intransponível na Zona 1: um personagem na Fase 52 travava por volta da profundidade 22 e não conseguia alcançar o Guardião na profundidade 25 (cujo HP chegava a ~40× o de um inimigo comum de campanha na mesma fase, somando o expoente de profundidade ao antigo `hpMult: 6.0`). Com os dois ajustes combinados, o Guardião da Zona 1 cai para ~7× o HP de um inimigo comum de campanha equivalente, viável para personagens em torno da Fase 50-52 com equipamento razoável.

### D. A Cidadela Submersa — 6 Distritos (`sunkenCitadelFormulas.ts`, `SubmersaPanel.tsx`)
Desbloqueada ao alcançar a **Fase 50** (`isFullDepthsUnlocked`, mesma condição que libera As Profundezas completas — revisado na v10.7.0; antes exigia `historicalMaxDepth >= 51`, entrar na Zona 3 do mergulho). O gate original criava um ciclo vicioso: o Traje de Mergulho, que ajuda a atravessar a Pressão pós-F50, só ficava disponível depois de restaurar a Doca Batial, e a Doca Batial só ficava acessível depois de já ter alcançado a própria Zona 3 — um requisito praticamente impossível de cumprir sem já ter os upgrades que ele deveria fornecer. Pátio 2×3 clicável (`SubmersaSpriteStage.tsx`, mesmo esqueleto de `CitadelSpriteStage.tsx`, com overlay de água CSS animando a altura durante a drenagem em tempo real). 6 distritos (`DISTRICT_IDS`): Doca Batial (⚓), Salão dos Ecos (🏛️), Forja Encharcada (⚒️), Arquivo Submerso (📚), Templo da Maré (🕍), Trono Afundado (👑) — adjacência ortogonal fixa (`DISTRICT_ADJACENCY`) usada na fórmula de eficácia (Seção E). Cada distrito: Alagado → drenagem (Pérolas+Coral+horas reais, `DISTRICT_DRAIN_COST`) → Restaurado I automático (função principal ativa + 1º slot de Eco) → Restauração II/III opcionais (`getRestorationCost`, mais slots e bônus de distrito). Modal de distrito (`DistrictModal.tsx`) mostra soquetes circulares de Eco (vazio = tracejado, ocupado = glifo de vocação).

### E. Os Ecos Afogados — Simulação de População
Resgate (`rescueEcho`): 10%/profundidade concluída na Zona 3+ (máx. 2/descida, `ECHO_RESCUE_CHANCE_PER_DEPTH`/`ECHO_RESCUE_MAX_PER_DIVE`), ou 100% ao concluir uma drenagem de distrito. Cada `DrownedEcho` tem uma vocação (`EchoVocation`: `fisher`/`diver`/`scribe`/`warden` — afinidade ×1.5 no distrito primário via `VOCATION_PRIMARY_DISTRICT`, ×1.25 no secundário) e um traço (`EchoTraitId`, 12 traços, raridade 60/30/10%: Constante, Insone, Contador de Histórias, Tímido, Nostálgico da Maré, Filho da Tempestade, Gêmeo de Eco, Mão Dupla, Memória de Ferro, Farol Humano, Voz do Coro, Coração Partido). **Fórmula de eficácia** (`calculateEchoEfficacies`): `Base(distrito) × Afinidade × (1+Contribuição Própria) × (1+Soma dos Vizinhos) × (1+Bônus do Salão)`, multiplicador com teto `EFFICACY_MULTIPLIER_CAP = 2.5`. Bônus do Salão dos Ecos: 6%/Eco alocado lá (12% se Voz do Coro), teto 24% — Restauração III soma +2%/Eco "descansando" (sem distrito), teto +8%. **Coração Partido**: 7 dias de descanso (`BROKEN_HEART_HEAL_MS`) antes de voltar a contribuir; realocar um Eco com esse traço para um distrito diferente reinicia o prazo (aviso explícito na UI antes de confirmar). Cap do elenco: `getEchoRosterCap(nível do Salão) = 12 + 2×nível` (12/14/16/18) — corrigido de uma constante fixa em 16 na revisão pós-lançamento, que ignorava a intenção original de "cap inicial 12, expansível".

### F. Ciclo de Marés e Bênçãos do Templo
`getTidePhase`: relógio determinístico de 6h real (`Date.now()`), 3h Maré Baixa/3h Maré Alta — acelerado para um ciclo de 1h nas sextas-feiras (`isMareVivaActive`, evento "Maré Viva", completando o calendário semanal ao lado da Lua de Sangue/domingo e da Convergência/quarta). Maré Baixa: +50% pesca, −20% custo de drenagem, −10% Pressão nas Profundezas. Maré Alta: −25% pesca, +50% Coral de inimigos aquáticos, ativa as Bênçãos do Templo (3 cards de escolha única: dano/drop/produção, `TIDE_BLESSINGS`) — Restauração III do Templo permite uma 2ª Bênção simultânea a 50% de força.

### G. Câmara de Gravação, Runas Abissais e Palavras Rúnicas (`runeFormulas.ts`)
12ª construção da Cidadela Astral (`EngravingChamberPanel.tsx`, 5 níveis) — perfuração de soquetes em equipamento pesado (`getMaxSocketsForSlot`, cabeça atinge 3 soquetes no Nível 5) e engaste de runas dropadas nas Profundezas (8% flat por abate, sem influência de Sorte, `DIVE_RUNE_DROP_CHANCE`). **9 famílias base** (`RUNE_FAMILIES`, 3 tiers cada — Ur/Kar/Sol/Vin/Mar/Nix/Lum/Dol/Fen, cada uma com um efeito secundário exclusivo de Tier III consumido em `CombatFSM.ts` via `hasRuneSecondaryFlag`). **9 Runas Primordiais** (`PRIMORDIAL_RUNES`, tier único, não empilham entre si no personagem): Thal (Guardião 1), Nereh (comprada no Templo, 200 Pérolas), Vrak (Guardião 2), Ciss (Carpideira do Sal, drop raro), Morvo (Guardião 3), Ecoh (12º Eco resgatado), Faro (pesca ativa), Levh (1ª morte do Leviatã), Umbra (Fossa Z4, drop raro). **Palavras Rúnicas** (`RUNEWORD_CATALOG`, 9 receitas, só na Câmara Nível 5): gravar a sequência exata de runas certa (`engraveRuneword`) num item com soquetes suficientes sobrescreve `item.socketedRunes` e ativa `activeRuneword` — `StatEngine.ts` passo 4.7 checa `getActiveRuneword(item)` antes de somar runas individuais, substituindo pelo efeito fixo. `undoRuneword` devolve as runas ao cofre intactas.

**Ícones de runa — arte definitiva integrada (pós-lançamento)**: a Fase de Lançamento usava só glifo Unicode + cor CSS por família/tier (`getRuneVisual`, `itemVisuals.tsx`), com a nota explícita de que a arte real viria depois (Anexo 3, §1.6). As duas spritesheets planejadas (`runes_base.png` — 9 famílias — e `runes_primordial.png` — 9 primordiais —, 1024×1024 em grid 3×3, ordem de leitura em linha) chegaram e foram ligadas via `IconSprite.tsx` (`src/components/shared/`), um primo do `EvolutionSprite.tsx` (Seção 18.H) que recorta por **índice de célula** em vez de por tier de evolução, reaproveitando a mesma remoção de fundo por chroma key (`imageBackgroundStrip.ts`). `getRuneSpriteInfo(runeId)` mapeia cada runa à sua célula (`RUNE_FAMILY_IDS.indexOf(family)` para as base; uma lista de ordem explícita, `PRIMORDIAL_RUNE_ORDER`, para as primordiais — não têm campo de ordenação no catálogo). O componente `RuneChip` (`itemVisuals.tsx`, exportado para substituir o markup antes duplicado em 3 lugares: picker/soquetes da Câmara e os dois modais de detalhe de item em `GameUI.tsx`) exibe o sprite real com fallback automático para o glifo/cor de sempre caso a imagem falhe ao carregar — mesma garantia que `EvolutionSprite` já dá para as construções.

### H. Set Abissal (`sunkenCitadelFormulas.ts`)
...
5.  **O Coração do Ciclo** — Fúria do Ciclo (+2%/10s de dano e velocidade, cap +200%) + Vagalhão a cada 15s **não-interrompível**, com 30% de chance de atordoar o jogador por 2s ao completar (`LEVIATHAN_PHASE5_STUN_CHANCE`) — o único stun aplicado ao jogador em todo o jogo, alvo real da imunidade da Palavra Rúnica ÂNCORA DO MUNDO (antes uma cláusula catalogada sem nenhum consumidor).

Recompensas: banca Pérolas/runa por fase vencida (mesmo em derrota, 1x/semana); 1ª morte na vida revela CORAÇÃO DO LEVIATÃ, concede a Runa Primordial Levh e um título honorífico, dispara a cutscene (Seção J); mortes seguintes garantem peça(s) do Set Abissal; full clear em 1 tentativa concede bônus extra de Pérolas (`leviathanFastestFullClear`).

### J. "O Coro e o Caco" — Primeira Cutscene do Jogo (`LoreCutscene.tsx`)
...
### K. Codex e Bestiário Atualizados (`codexData.ts`, `bestiaryFormulas.ts`)
21 novas entradas de `bestiaryEntries` cobrindo todo o elenco do Abismo (Litoral, 4 zonas das Profundezas, os 3 Guardiões, o Leviatã), desbloqueadas por `killed(ctx, enemyId)`. O cálculo do bônus de dano do Bestiário (`StatEngine.calculateBestiaryDamageMultiplier`) e a UI da aba 🐉 Bestiário passaram a compartilhar uma única fonte de verdade (`BESTIARY_PHASE_GROUPS`, `bestiaryFormulas.ts`, agrupamento por ID de inimigo) — antes cada um tinha sua própria lista hardcoded, e a UI usava um recorte posicional fixo do array `ENEMY_TYPES` que ignorava tudo adicionado após os 7 biomas originais + Litoral + Zona 1, fazendo com que abates do Abismo nunca contribuíssem para o bônus de dano e a UI mostrasse só 2 dos 6 grupos novos. Além do Bestiário, ~21 entradas novas foram adicionadas às demais categorias do Codex (3 de Cosmologia, 2 de Facções, 4 de Personagens — bio mítica dos 3 Guardiões e do Leviatã, distinta da entrada técnica do Bestiário —, 5 de Eventos e 7 de Locais), fechando uma lacuna em que a expansão inteira não tinha nenhuma presença fora do Bestiário.

### L. Correções de Isolamento (pós-lançamento)
Os 21 ids exclusivos do Abismo (`ABYSS_ENEMY_IDS`, `CombatFSM.ts`) vazavam para os sorteios aleatórios de inimigo comum/chefe tanto da Torre Infinita quanto do Pandemônio (`setupEnemyForLevel` filtrava `ENEMY_TYPES` só por `id.startsWith('boss_')`, sem excluir os ids do Abismo) — corrigido adicionando `!ABYSS_ENEMY_IDS.has(e.id)` aos 4 pontos de sorteio (comum/chefe × Torre/Pandemônio). O Coral também era zerado a cada 60s sempre que havia Expedições da Cidadela alocadas — `tickCitadelProduction` reconstruía o objeto `materials` sem espalhar (`...materials`) os campos já existentes, corrigido no bloco do Quartel de Expedições.

### M. Ajustes de UI/QoL e Revisão das Runas Primordiais (v10.5.0, revisão adicional pós-lançamento)
Rodada de ajustes de interface e uma revisão de design das Runas Primordiais, feita após o lançamento inicial do sistema de runas:

*   **Layout mobile da Câmara de Gravação**: as linhas de soquete (`EngravingChamberPanel.tsx`) ganharam `flexWrap` — em telas estreitas, o grupo de botões "⚗️ Extrair"/"💥 Remover" quebra para a linha abaixo em vez de vazar do card, e o botão "← Voltar" dos cabeçalhos (Vista 2 e Vista 3 do fluxo de gravação) recebeu `flexShrink: 0` para nunca ficar deslocado por um nome de item longo.
*   **Fim do flash de ícone de fallback nas runas**: `App.tsx` passou a pré-processar (`getTransparentImageUrl`) as duas spritesheets de runa (`runes_base.png`/`runes_primordial.png`) assim que o app monta, no mesmo padrão já usado para os sprites da Cidadela (Seção 18.H) — elimina o flash do glifo genérico ao abrir detalhes de item com runa socketada, tanto no Inventário quanto na Câmara.
*   **Nova aba "Runas" no Codex** (`codexData.ts`): 19 entradas (1 visão geral sempre visível + 9 famílias base agrupadas por Tier I-III, já que os 3 tiers compartilham a mesma mecânica + 9 Primordiais individuais), desbloqueadas via `runeInventory[id] > 0`, no mesmo padrão de `killed()` já usado pelo Bestiário.
*   **Nova aba "Runas" no Inventário** — ver Seção 5.E.
*   **Consistência das barras de recursos**: a barra superior fora da Cidadela (Ouro/Chaves/Chaves Evoluídas) passou a exibir só ícone+quantidade (o nome vira `title`/tooltip), no mesmo padrão que a barra de recursos já usava dentro da Cidadela (`CITADEL_HEADER_RESOURCES`, `GameUI.tsx`). O cabeçalho de recursos da Câmara de Gravação deixou de exibir Ouro (mantém Madeira/Pedra/Coral/Pérolas) — o Ouro continua sendo cobrado normalmente nas ações da Câmara, só não aparece mais no resumo do topo.
*   **Mergulhos Rasos reorganizados** (`AbyssPanel.tsx`): título + resumo compacto de 1 linha + estatísticas (chaves/recorde) + botão de mergulho/subir em destaque logo no topo do painel; o texto de lore completo, o aviso de chaves insuficientes e as regras de "morte" (percentual mantido) foram agrupados no rodapé como informações de referência.

**Runas Primordiais deixam de ser limitadas a 1 cópia por conta (exceto Nereh)**: investigação motivada por um relato do jogador revelou que 7 das 9 Primordiais tinham uma trava de concessão única (`faroGranted`, `guardiansDefeated[zona]`, marco exato `newLifetime === 12`, `leviathanKillCountLifetime <= 0`), enquanto Ciss e Umbra já eram drops repetíveis por RNG (0.5%/0.3% por abate) sem nenhuma trava — inconsistência não intencional de design. Correção:
    *   **Faro** (`useGameStore.ts`, `resolveActiveFishing`): passa a ser determinística — a cada 100 acertos perfeitos acumulados (100, 200, 300...) concede mais uma cópia, calculado por `Math.floor(faroPerfectCatches / FARO_PERFECT_CATCHES_REQUIRED)` contra um novo campo `faroGrantedCount` (`Character.coastal`), que substitui a antiga flag booleana `faroGranted` como fonte de verdade (mantida só por compatibilidade com saves antigos). A UI (`CoastalPanel.tsx`) volta a mostrar o progresso `X/100` corretamente reiniciado a cada cópia obtida, somado à contagem total já conquistada.
    *   **Ecoh** (`rescueEcho`): 1ª cópia continua garantida no 12º resgate; resgates seguintes têm 0.4% de chance de repetir.
    *   **Thal / Vrak / Morvo** (`useDiveStore.ts`, bloco `killedGuardian`): 1ª cópia continua garantida na 1ª morte do respectivo Guardião de Zona; mortes seguintes do mesmo Guardião têm 0.5% de chance de soltar outra cópia.
    *   **Levh** (`killLeviathan`): 1ª cópia continua garantida na 1ª morte do Leviatã; mortes de repetição (que já concediam Pérolas) têm 0.5% de chance de soltar outra cópia.
    *   **Nereh** permanece de compra única intencional (Templo da Maré, guarda de quantidade com comentário explícito no código-fonte) — exceção confirmada com o jogador, não alterada.
    *   A regra "1 por soquete NO PERSONAGEM INTEIRO" (`runeFormulas.ts`) não muda — continua impedindo equipar 2 cópias simultâneas da mesma Primordial —, mas múltiplas cópias no cofre (`runeInventory`) agora são suportadas e pretendidas para todas as Primordiais, exceto Nereh.

### N. Endgame, Guia, Correção de Sprites e Litoral como Bloco de Fase (v10.6.0)
Rodada de ajustes de endgame, uma auditoria completa de assets e uma revisão de design do Litoral, todas motivadas por testes de jogo do usuário após a v10.5.0:

*   **Bônus permanente de Transcendência**: novo `StatEngine.getTranscendenceBoost(character)` — `1 + transcendenceCount × 0.05` — aplicado como fator multiplicativo **por fora** do pool aditivo de `damageMultiplierPct`/`maxHpPct`/`maxManaPct` (mesmo espírito de `alma_avatar`/Ecoh), garantindo que o bônus tenha impacto real mesmo depois de perder equipamento e Ascensões ao transcender. Consumido nas 3 cadeias de `setDamageMultiplier` e no `hpBoost`/`manaBoost` do `CombatFSM.ts`, e em `manaFormulas.ts`/`GameUI.tsx` (`StatisticsPanel`) para a mana/vida exibidas fora de combate baterem com o valor real de combate. Cada Transcendência soma mais +5%, empilhando (2 ciclos = +10%, 3 = +15%...).
*   **Correção do Desafio Diário (bug de inflação de XP/nível)**: mesma classe de bug já corrigida na Torre Infinita — um personagem de fase alta podia farmar a Fase Espelho (10-19, fácil demais para o nível real) e ganhar XP/nível/pontos sem limite. Corrigido replicando o teto da Torre: `getSavedLevelBeforeDailyChallenge()` (novo getter em `useGameStore.ts`, ao lado do já existente `savedLevelBeforeTower`) trava o XP por abate em 1% do XP necessário para o nível congelado na entrada do desafio (`CombatFSM.ts`, branch `isDaily` ao lado do `isTower` já existente).
*   **Estatísticas — contador de Faro corrigido**: `StatisticsPanel` (`GameUI.tsx`) mostrava `faroPerfectCatches` bruto (`137/100`) em vez do `% FARO_PERFECT_CATCHES_REQUIRED` já usado em `CoastalPanel.tsx` desde a correção da v10.5.0 — unificado para o mesmo padrão, com a contagem de cópias já obtidas (`faroGrantedCount`) somada ao lado.
*   **Guia reorganizado**: a sub-aba "🏰 Sistemas" (que tinha crescido para 20 tópicos após a v10.5.0) ganhou um segundo nível de sub-navegação — 6 categorias por proximidade de tema (🏰 Cidadela, ⚔️ Equipamento, 🚀 Endgame, 🌙 Eventos, ⚙️ QoL, 🌊 Cidadela Submersa), mesmo padrão visual do picker "⚔️ Classes" já existente (`GuidePanel`, `GameUI.tsx`). Também foi confirmado e preenchido um vazio real: nenhum conteúdo da v10.0.0 "Cidadela Submersa" estava documentado no Guia (Relíquia Ativa/Provações do Vácuo/Convergência da v9.0.0 já estavam, ao contrário do que se suspeitava) — adicionados 8 novos tópicos cobrindo Litoral, Profundezas, Cidadela Submersa, Ecos Afogados, Ciclo de Marés, Câmara de Gravação/Runas/Palavras Rúnicas, Set Abissal e Leviatã.
*   **Consistência de título de seção**: Forja Mística (`ForgeView.tsx`) usava uma classe própria (`.forge-title`, maior, sem sublinhado) em vez da `.section-title` compartilhada usada em Equipamentos/Codex/Torre/Loja/Guia — unificado (classe morta removida do CSS). A aba Abismo (`AbyssPanel.tsx`) não tinha nenhum título de seção — adicionado no mesmo padrão (wrapper com `borderBottom` + `h2.section-title` com borda própria suprimida, técnica já usada por Equipamentos/Torre/Loja/Guia para evitar borda dupla). O botão "🕸️ Recolher a Rede" do Litoral (`CoastalPanel.tsx`), antes numa linha compartilhada com o botão de melhoria da Doca (alinhado à esquerda), passou a ficar centralizado sozinho, igual ao botão "🎣 JOGAR A LINHA" da Pesca Ativa.
*   **Changelog**: removida do modal de atualização (`App.tsx`) a frase sobre os sprites definitivos ainda estarem "a caminho" — detalhe técnico interno que deixou de ser verdade assim que os ~40 sprites da expansão foram concluídos pelo usuário, e que não precisa aparecer para o jogador.
*   **Auditoria e correção do carregamento de sprites (Zonas 2-4, Leviatã, backgrounds)**: o usuário completou todos os ~40 assets planejados no Anexo 3 (`Assets_UI_e_Cutscene.md`), mas só a Zona 1 (Litoral + Recife Partido) havia sido conectada ao carregador do Phaser quando a v10.0.0 foi lançada. `CombatScene.ts` ganhou `load.image`/`makeTextureTransparent`/entradas de fallback para os 12 inimigos das Zonas 2-4, o Leviatã, e os 4 backgrounds (`abyss_z2/z3/z4_background.png`, `throne_arena_background.png`) que nunca tinham sido carregados — nenhum deles aparecia com arte real até esta correção, apesar dos arquivos existirem desde antes. A seleção de background durante um Mergulho passou a trocar por Zona (`getZoneForDepth`) em vez de sempre usar a arte da Zona 1, e a luta do Leviatã ganhou sua própria arena (`throne_arena_background`, antes sem tratamento de background dedicado). Corrigido também um bug de digitação: `leviathan_spawn` referenciava a textura `enemy_leviathan_spawn` (arquivo inexistente) em vez de `boss_leviathan_spawn` (o arquivo real). Auditoria final: 41/41 arquivos de asset da expansão conferidos e corretamente conectados.
*   **Background da Cidadela Submersa calibrado**: `submersa_background.png` foi conectado ao `SubmersaSpriteStage.tsx` (antes só um gradiente CSS). A primeira versão gerada tinha 5 clareiras em formato de "X" em vez da grade 2×3 (6 clareiras) que o código espera — identificado por análise de posições, corrigido pelo usuário. Depois de recebida a versão com 6 clareiras corretas, a imagem foi recortada (crop quadrado centralizado 750×750 de 1024×1024, reescalada de volta a 1024×1024 sem distorção) para espalhar melhor os espaços, e `MARKER_POSITIONS` foi recalibrado por análise de pixel (detecção das regiões claras dos 6 círculos) para bater exatamente com os novos centros (colunas ~21/50/79%, linhas ~28/72%, ajustado de 18/50/82% e 32/72%).
*   **Litoral como bloco de fase dedicado**: antes, encontros aquáticos do Litoral interrompiam as Fases 1-10 por um roll probabilístico por combate (15%, 30% às sextas via "Maré Viva"), trocando o cenário/inimigo no meio de uma fase e quebrando o ritmo do sidescroll. Substituído por um novo módulo compartilhado, `CAMPAIGN_BIOME_BLOCKS`/`getLitoralBlockIndexForAscension` (`abyssFormulas.ts`): um dos 3 blocos elegíveis de 5 fases (Floresta 6-10, Deserto 11-15, Gelo 16-20 — o Bosque Sussurrante, Fases 1-5, fica fora do sorteio porque o Litoral só desbloqueia ao completar a Fase 2) vira tema Litoral **por inteiro**, sorteado deterministicamente a cada Ascensão (mesmo personagem/Ascensão = sempre o mesmo bloco; próxima Ascensão sorteia de novo, mesmo princípio do sorteio semanal de chefe da Convergência). Dentro do bloco sorteado, o pool de inimigos comuns vira `wreck_crab`/`drift_jelly`/`slime_moray` e o Eco Afogado (`drowned_echo`) passa a ser o chefe garantido da 5ª fase do bloco, substituindo o chefe normal daquele bioma. `CombatFSM.ts` (pool de inimigos/chefe) e `CombatScene.ts` (background) consomem a mesma fórmula, nunca divergindo. A função `isMareVivaActive()` (usada só pela interrupção antiga) foi removida por ficar sem uso.
*   **Rebalanceamento de Fragmentos de Batisfera e iscas**: o peso do Fragmento de Batisfera na tabela de captura (`getFishingTable`, `abyssFormulas.ts`) era baixo demais (2/4/8 num pool de 100, conforme a isca) — reportado pelo usuário como excessivamente lento para montar uma Chave de Mergulho (5 fragmentos). Aumentado em 4.5× para as 3 iscas (Carne 2→9, Luminosa 4→18, Abissal 8→36), absorvendo a diferença principalmente do peso de peixe comum — o que também torna o viés de cada isca muito mais perceptível na prática (antes 50/38/40 de peixe comum entre as 3 iscas, quase indistinguível; agora 43/24/20). Nenhuma mudança em `BATHYSPHERE_FRAGMENTS_PER_KEY` (continua 5) nem nas capturas/hora da Doca — o problema era a raridade por captura, não o throughput.

### O. Reformulação do Acesso à Cidadela Submersa e Ajustes de Retenção (v10.7.0)
Rodada motivada por testes de jogo do usuário após a v10.6.0: um personagem na Fase 52 não conseguia sequer alcançar o Guardião da Zona 1 (profundidade 25), travando por volta da profundidade 22, o que tornava o desbloqueio da Cidadela Submersa (então preso à Zona 3, prof. 51+) praticamente inatingível. Ver Seções 9.A, 19.C e 19.D para a documentação técnica completa.

*   **🔱 Desbloqueio da Cidadela desvinculado da profundidade**: o gate real (`startDistrictDrain`, `useGameStore.ts`), a UI da sub-aba Cidadela (`AbyssPanel.tsx`) e a entrada correspondente do Codex (`codexData.ts`) passaram de `historicalMaxDepth >= 51` para `isFullDepthsUnlocked(highestStageReached)` (Fase 50) — a mesma condição que já libera As Profundezas completas. Elimina o ciclo vicioso em que o Traje de Mergulho (que ajuda a atravessar a Zona 3) só ficava disponível depois de restaurar a Doca Batial, e a Doca Batial só ficava acessível depois de já ter alcançado a própria Zona 3. Textos descritivos em `App.tsx`, `GameUI.tsx` e `EchoRosterDrawer.tsx` atualizados para refletir o novo requisito.
*   **⚔️ Guardião da Zona 1 rebalanceado**: `hpMult` de `6.0` para `3.0` e `dmgMult` de `1.8` para `1.5` (`ZONE_GUARDIANS`, `abyssFormulas.ts`) — os Guardiões das Zonas 2 e 3 não foram alterados. Combinado com a suavização do expoente de HP por profundidade (abaixo), o Guardião da Zona 1 cai de ~40× para ~7× o HP de um inimigo comum de campanha na mesma fase.
*   **📉 Curva de HP por profundidade suavizada**: expoente de `getDiveEnemyHP` (`abyssFormulas.ts`) reduzido de `1.14` para `1.09`, afetando toda a progressão de dificuldade do mergulho (não só o Guardião) — o crescimento de HP entre as profundidades 1-24 também estava contribuindo para a parede de dificuldade relatada, antes mesmo do jogador alcançar o chefe.
*   **🦪 Retenção de Pérolas/Coral na Ascensão reduzida para 50%**: antes sobreviviam 100% intactas a cada Ascensão; agora retêm metade do saldo acumulado (`performPrestige`, `useGameStore.ts`), ainda bem acima dos 2% dos materiais de Expedição comuns, mas o suficiente para evitar acúmulo excessivo entre runs. Ver Seção 9.A.
*   **🎨 Fonte dos botões da aba Abismo corrigida no mobile**: os rótulos dos botões de sub-aba (Litoral/Profundezas/Cidadela, `AbyssPanel.tsx`) usavam `0.8rem`, fazendo o texto "Profundezas" estourar a largura do botão em telas estreitas. Reduzido para `0.62rem`, o mesmo tamanho já usado pelos botões de seleção de ramificação da Torre Infinita (`TowerPanel.tsx`).

### P. Aba Guia Completa, Indicador de Bônus de Transcendência e Visão Geral da Cidadela (v10.7.1)
Rodada de polimento motivada por uma nova rodada de testes do usuário sobre a aba Guia (`GuidePanel`, `GameUI.tsx`): conteúdo desatualizado/incompleto, um bug de layout no mobile, e a falta de um indicador numérico para o bônus permanente de Transcendência (introduzido na v10.6.0, Seção 19.N) e para o nível da Câmara de Gravação na Visão Geral da Cidadela.

*   **👑 Nova categoria "Elites" no Guia**: inserida entre "⚙️ QoL" e "🌊 Cidadela Submersa" na sub-navegação de Sistemas (`guideSystemsSubTab`). Antes, o único tópico sobre Elites ("👑 Elites e 🌕 Lua de Sangue (v8.0.0)", categoria Eventos) documentava apenas 4 dos 12 afixos hoje implementados em `CombatFSM.ts` (`eliteAfix`) — faltavam os 5 afixos originais (Enfurecido, Blindado, Vampírico, Volátil, Regenerador) e os 3 da v10.1.0 (Abissal, Sifonador, Bioluminescente), além de mecânicas gerais (dano 3× em todo ataque de Elite, drop de material em dobro, chance de spawn variando por modo — Torre Infinita, Profundezas, campanha) e contra-jogo (Selo do Caçador de Elites, Runas Nix `eliteDamagePct`). A lista antiga foi removida do card de Eventos, que passou a falar só da 🌕 Lua de Sangue.
*   **🪬 Câmara de Gravação adicionada à lista de construções da Cidadela (Guia)**: a categoria "🏰 Cidadela" do Guia listava as 10 construções da Cidadela Astral mas não incluía a Câmara de Gravação (12ª construção, v10.0.0) — adicionado um item na lista com referência à explicação completa já existente na categoria "🌊 Cidadela Submersa" (evita duplicar conteúdo).
*   **🐛 Overflow mobile na fileira de categorias do Guia corrigido**: o container de categorias usava `display: grid` com `gridTemplateColumns: repeat(3, 1fr)` combinado com a classe compartilhada `.tab-btn` (`white-space: nowrap`, `src/index.css`). Em CSS Grid, um item não encolhe abaixo do tamanho do seu conteúdo quando o texto não pode quebrar linha — rótulos longos como "🌊 Cidadela Submersa" forçavam a grade (e, por consequência, o container pai, que só tem `overflow-y: auto`) a ficar mais larga que a viewport no mobile, vazando para fora da tela. Corrigido trocando para `display: flex` com `flexWrap: wrap`, `whiteSpace: 'normal'` e `minWidth` por botão (só nos estilos inline deste container específico — a classe global `.tab-btn`, usada em várias outras fileiras de aba de uma linha só, não foi alterada). A correção já nasceu preparada para a 7ª aba (Elites) adicionada no mesmo momento.
*   **🌌 Indicador de bônus de Transcendência**: o `TranscendencePanel` (`GameUI.tsx`) já mostrava a contagem de Transcendências realizadas, mas nunca o percentual resultante do bônus permanente (`StatEngine.getTranscendenceBoost`, +5%/ciclo — Seção 19.N). Adicionada uma nova linha "Bônus Atual (Dano/Vida/Mana): +X%" logo abaixo da contagem, calculada via `Math.round((StatEngine.getTranscendenceBoost(character) - 1) * 100)` — deriva o percentual da própria fórmula em vez de duplicar o número mágico `5` inline, permanecendo sincronizada se a fórmula mudar no futuro.
*   **🏛️ Câmara de Gravação na Visão Geral da Cidadela**: `CitadelOverview.tsx` (aba "Visão Geral" da Cidadela) tem uma lista `buildings` com nome/ícone/nível de cada construção, usada para renderizar a lista de status — a Câmara de Gravação (`citadel?.engravingChamber`) existia no estado e tinha painel próprio (`EngravingChamberPanel.tsx`) e sprite (`BUILDING_SPRITE_SRC.engravingChamber`), mas nunca tinha sido adicionada a esse array, então não aparecia na Visão Geral com seu nome/nível como as outras 10 construções. Corrigido com uma entrada adicional no array `buildings`.
...
Rodada de ajustes de interface e uma revisão de design das Runas Primordiais, feita após relatos do jogador sobre o sistema de runas introduzido na v10.0.0. Ver Seção 19.M para a documentação técnica completa.
...
### Versão 10.9.0 "QoL das Cidadelas e Economia de Energia" (Atual)
...
Rodada de polimento da aba Guia motivada por uma nova sessão de testes do usuário: conteúdo incompleto/desatualizado, um bug de overflow no mobile, e dois indicadores numéricos faltando (bônus de Transcendência, nível da Câmara de Gravação na Visão Geral da Cidadela). Ver Seção 19.P para a documentação técnica completa.
...
Ajustes de endgame (Transcendência, Desafio Diário), auditoria e correção do carregamento de ~40 sprites concluídos pelo usuário para a v10.0.0, reorganização do Guia, e uma revisão de design do Litoral (bloco de fase dedicado em vez de interrupção por encontro) e de sua economia de Fragmentos de Batisfera. Ver Seção 19.N para a documentação técnica completa.
